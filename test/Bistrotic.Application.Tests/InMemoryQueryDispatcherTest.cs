    ...
    public class InMemoryQueryDispatcherTest
    {
        ...
        public async Task Dispatch_query_check_return_values()
        {
            ...
            (await dispatcher.Dispatch<Query1, int>(
                    new Envelope<Query1>(new Query1(), new MessageId(), new UserName("User 1"))
                    ).ConfigureAwait(false))
                .Should().Be(1);
            (await dispatcher.Dispatch<Query2, string>(
                    new Envelope<Query2>(new Query2(), new MessageId(), new UserName("User 2"))
                    ).ConfigureAwait(false))
                .Should().Be("2");
            (await dispatcher.Dispatch<Query3, Guid>(new Envelope<Query3>(new Query3(), new MessageId(), new UserName("User 3"))
                ).ConfigureAwait(false))
                .Should().Be(new Guid("66CABB1C-18E3-4E26-AE0F-EA603D9F11FB"));
            var query = new Envelope<Query4>(new Query4(), new MessageId(), new UserName("User 4"));
